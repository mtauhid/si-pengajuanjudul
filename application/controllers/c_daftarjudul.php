...
class c_daftarjudul extends CI_Controller{
  ...
  /*validasi jika session dengan level tidak sesuai
	...
	function index(){
		...
		$this->load->database();
		$jumlah_data = $this->m_daftarjudul->Page_jumlahreview('tb_judul');
		$this->load->library('pagination');
		$config['base_url'] = base_url().'c_daftarjudul/v_daftarjudul/index';
		$config['total_rows'] = $jumlah_data;
		$config['per_page'] = 5;
		$from = $this->uri->segment(4);
		$this->pagination->initialize($config);		
	    $data['tb_judul'] = $this->m_daftarjudul->Page_datareview('tb_judul',$nip,$config['per_page'],$from);
	    $this->load->view('template/header');
	    $this->load->view('template/sidebar');
		$this->load->view('v_daftarjudul',$data);
...
function hapus($id_judul){
		...
		redirect('c_daftarjudul/index');
	}

	public function edit($id_judul){
		$where = array('id_judul' => $id_judul);
		$data['tb_judul'] = $this->m_daftarjudul->edit_data($where,'tb_judul')->result();
		$this->load->view('v_daftarjudul',$data);
	}
