...
class c_laporanJudulFix extends CI_Controller{
  ...
  //validasi jika session dengan level tidak sesuai
    if ($this->session->userdata('level') == "dosen") {
      redirect('Dashboard');
		...
		$this->load->model('m_laporanJudulFix');
	...
	function index(){
		...
		$this->load->database();
		$jumlah_data = $this->m_laporanJudulFix->Pagejumlah_KirimJudul('tb_judul');
		$this->load->library('pagination');
		$config['base_url'] = base_url().'c_laporanJudulFix/v_laporandaftarjudulFix/index';
		$config['total_rows'] = $jumlah_data;
		$config['per_page'] = 9000;
		$from = $this->uri->segment(4);
		$this->pagination->initialize($config);		
		$data['tb_judul'] = $this->m_laporanJudulFix->PageReview_judul('tb_judul',$config['per_page'],$from);
		$this->load->view('template/header');
		$this->load->view('template/sidebar');
		$this->load->view('v_laporandaftarjudulFix',$data);
